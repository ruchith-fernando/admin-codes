...
<div class="table-responsive font-size">
    <table class="table table-bordered table-striped align-middle text-start">
        ...
        <?php while($row = $result->fetch_assoc()) { 
            ...
            $hris = strtolower($row['hris_no']);
            $employee_name = strtolower($row['full_display_name']);
            $voice_data = strtolower($row['voice_data']);

            if (strpos($hris, 'police') !== false || strpos($employee_name, 'police') !== false || strpos($voice_data, 'police') !== false) {
                continue; // skip rows where 'police' appears anywhere
            }
...
$(document).ready(function(){
    ...
    //     window.location.href = "?page=" + page + "&search=" + encodeURIComponent(currentSearch);
    // });
});
</script>
