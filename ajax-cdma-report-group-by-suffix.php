<?php
// ajax-cdma-report-group-by-suffix.php
// Grouped summary by ALLOCATED TO (joined via trimmed connection_no) for a selected month (YYYY-MM).
...
try {
    ...
    // ----------------------------------------------------------------------
    // 2) Fetch rows and JOIN details to get allocated_to using TRIMMED connection_no
    //    (remove last character), then group by allocated_to.
    // ----------------------------------------------------------------------
    $sqlRows = "
      SELECT
        m.id AS upload_id,
    ...
    while ($r = mysqli_fetch_assoc($resRows)) {
        // Keep your trimming rule for display parity (not needed for grouping now)
        $connNoRaw = norm_space($r['connection_no'] ?? '');
        $trimmed   = (strlen($connNoRaw) > 0) ? substr($connNoRaw, 0, -1) : $connNoRaw;

        $allocatedTo = norm_space($r['allocated_to'] ?? '');
        $groupKey    = ($allocatedTo !== '') ? $allocatedTo : 'Unknown';
    ...
    // Sort by allocated_to asc. To sort by total desc, replace with uasort comparing ['total'].
    ...
    <div class="table-responsive">
      <table class="table table-sm table-bordered align-middle">
        ...
        <tbody>
          <?php foreach ($groups as $g => $v): ?>
            <tr>
              <td class="text-nowrap"><?= e($g); ?></td>
