...
/* ✅ Build financial year month list:
   If you prefer dynamic-from-actuals only, switch to a DISTINCT query like your old file.
*/
$months = [];
...
/* ✅ Total branches (electricity) */
$total_branches_row = $conn->query("SELECT COUNT(*) AS total FROM tbl_admin_branch_electricity")->fetch_assoc();
$total_branches = (int)($total_branches_row['total'] ?? 0);
...
foreach ($months as $month) {
    ...
    /* Actuals: sum numeric (strip commas), ignore blank strings */
    $actual_row = $conn->query("
        SELECT SUM(CAST(REPLACE(TRIM(total_amount), ',', '') AS DECIMAL(15,2))) AS actual_amount
    ...
    if ($actual <= 0) continue; // match Security behavior: show months with actuals only

    /* Budget: monthly total */
    $budget_row = $conn->query("
        SELECT SUM(CAST(REPLACE(TRIM(amount), ',', '') AS DECIMAL(15,2))) AS budget_amount
        FROM tbl_admin_electricity_monthly_budget
        WHERE budget_month = '$month_esc'
    ")->fetch_assoc();
    $budget = (float)($budget_row['budget_amount'] ?? 0);

    /* Completion: distinct branches submitted actuals */
    $completed_row = $conn->query("
        SELECT COUNT(DISTINCT branch_code) AS completed
        FROM tbl_admin_actual_electricity
        WHERE month_applicable = '$month_esc' AND TRIM(total_amount) != ''
    ")->fetch_assoc();
    $completed = (int)($completed_row['completed'] ?? 0);
    ...
    ")->fetch_assoc();
    $selected = (isset($selected_row['is_selected']) && $selected_row['is_selected'] === 'yes') ? 'checked' : '';

    /* Difference/Variance (align with Security) */
...
/* optional focus highlight (kept consistent with Security) */
  ...
  <table class="table table-bordered table-sm text-center wide-table">
    ...
    <tbody>
      ...
      >
        ...
        <td>
          <?= number_format($row['actual'], 2) ?><br>
          <small class="<?= $row['completed'] < $row['total_branches'] ? 'text-danger' : 'text-muted' ?>">
            <?= $row['completed'].'/'.$row['total_branches'] ?> branches
          </small>
        </td>
