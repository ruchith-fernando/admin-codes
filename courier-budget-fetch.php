...
/* total branches */
$total_branches = 0;
$resTB = $conn->query("SELECT COUNT(DISTINCT branch_code) AS total FROM tbl_admin_budget_courier");
...
foreach ($months as $month) {
    ...
    if ($actual <= 0) continue; // skip empty months

    
    /* budget = yearly total for 2025 */
    static $yearly_budget = null;
    if ($yearly_budget === null) {
        ...
        ")->fetch_assoc();
        $yearly_budget = (float)($budget_row['budget_amount'] ?? 0);
    }
