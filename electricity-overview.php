  ...
  <div class="container-fluid">
    <div class="card shadow bg-white rounded p-4">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <h5 class="text-primary">Electricity Budget vs Actual</h5>
...
<!-- Unique remarks modal for Electricity -->
<div class="modal fade" id="electricityRemarksModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
...
<script>
(function(){
  const NS = '.page.electricity';

  // Base dir of this PHP (e.g. "/pages/")
  const BASE = '<?= htmlspecialchars(rtrim(dirname($_SERVER["PHP_SELF"]), "/")."/", ENT_QUOTES) ?>';

  // Two candidates: same folder, then site root (fixes 404 if file isn’t under /pages/)
  const FETCH_CANDIDATES = [
    BASE + 'electricity-budget-fetch.php',
    BASE.replace(/[^/]+\/$/, '') + 'electricity-budget-fetch.php' // parent dir -> "/"
  ];
  ...
  function loadElectricityReport(){
    $('#report-content').html(
    ...
    const go = (urls) => fetchWithFallback(
      ...
      () => {
        ...
        console.error('Electricity fetch failed on all paths:', urls);
      }
    );

    // Prefer the discovered working URL, else try both candidates
  ...
  // Open remarks (scoped to electricity modal)
  $('#contentArea').on('click' + NS, '.open-remarks', function(){
    const category = $(this).data('category');
    const record   = $(this).data('record');
    const $modal   = $('#electricityRemarksModal');
  ...
  $('#contentArea').on('click' + NS, '#save-remark', function(){
    ...
    $.post(BASE + 'save-remark.php', { category, record, comment, recipients, origin_page: 'electricity-overview.php' }, function(response){
  ...
  $('#contentArea').on('hidden.bs.modal' + NS, '#electricityRemarksModal', function(){
