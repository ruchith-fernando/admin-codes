...
$stmt = $mysqli->prepare("SELECT item_id, item_code, record_status FROM tbl_admin_item WHERE item_name=? LIMIT 1");
...
if ($row = $res->fetch_assoc()) {
  if ((int)$row['item_id'] === $item_id) {
    echo alertHtml('success', "Item Name is yours (editing).");
  } else {
    $code = htmlspecialchars($row['item_code']);
    $status = htmlspecialchars($row['record_status']);
    echo alertHtml('warning', "Item Name already exists (Code: <b>{$code}</b>, Status: <b>{$status}</b>).");
  }
} else {
