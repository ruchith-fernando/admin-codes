...
/* Use the same GL label you inserted into budget table */
// $gl_desc = 'NEWSPAPPER';
// $gl_desc_esc = mysqli_real_escape_string($conn, $gl_desc);
...
/* total branches (from budget table) */
$total_branches = 0;
$resTB = $conn->query("
    SELECT COUNT(DISTINCT branch_code) AS total
...
foreach ($months as $month) {
    ...
    /* -------- ACTUALS (from tbl_admin_actual_branch_gl_newspaper) --------*/
    $actual_row = $conn->query("SELECT
        SUM(debits) AS actual_amount
      FROM tbl_admin_actual_branch_gl_newspaper
      WHERE applicable_month = '$month_esc'
        AND debits <> 0
        AND UPPER(TRIM(COALESCE(tran_db_cr_flg,''))) = 'D'")->fetch_assoc();

    $actual = (float)($actual_row['actual_amount'] ?? 0);


    // skip empty months (keep negative months if any)
    if (abs($actual) < 0.005) continue;

    /* -------- BUDGET (monthly total from tbl_admin_budget_newspaper_branch) -------- */
    ...
    /* -------- COMPLETION (branches having any actual for the month) --------
       Uses brn_code if present, otherwise enterd_brn as fallback.
    */
    $comp_row = $conn->query("
        SELECT COUNT(DISTINCT COALESCE(NULLIF(brn_code,''), enterd_brn)) AS completed
        FROM tbl_admin_actual_branch_gl_newspaper
        WHERE applicable_month = '$month_esc'
