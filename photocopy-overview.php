...
<!-- Unique remarks modal for Photocopies -->
<div class="modal fade" id="photocopyRemarksModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
...
(function(){
  ...
  const FETCH_CANDIDATES = [
    BASE + 'photocopy-budget-fetch.php',
    BASE.replace(/[^/]+\/$/, '') + 'photocopy-budget-fetch.php'
  ];
  ...
  function recalcSelectionTotals(){
    ...
    $('#report-content .month-checkbox:checked').each(function(){
      ...
      sumBudget += parseNum($tds.eq(2).text()); // Budget col
      sumActual += parseNum($tds.eq(3).text()); // Actual col
    });
  ...
  function loadPhotocopyReport(){
    ...
    const go = (urls) => fetchWithFallback(
      ...
      () => {
        ...
        console.error('Photocopy fetch failed on all paths:', urls);
      }
    );
  ...
  // Open remarks (scoped to photocopy modal)
  $('#contentArea').on('click' + NS, '.open-remarks', function(){
    const category = $(this).data('category');
    const record   = $(this).data('record');
    const $modal   = $('#photocopyRemarksModal');
  ...
  $('#contentArea').on('click' + NS, '#save-remark', function(){
    ...
    $.post(BASE + 'save-remark.php', { category, record, comment, recipients, origin_page: 'photocopy-overview.php' }, function(response){
  ...
  $('#contentArea').on('hidden.bs.modal' + NS, '#photocopyRemarksModal', function(){
