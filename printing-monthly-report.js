...
$(document).ready(function () {
  ...
  /* -----------------------------
     Alerts (missing + provisions)
  ------------------------------*/
  function updateAlerts(res, month, missingSel, provisionSel) {
    let out = "";
  ...
  /* =========================================================
     VIEW MODE
     - when view opens, manual closes
  ========================================================== */
  $("#printing_month_view").on("change", function () {
    const month = ($(this).val() || "").trim();

    // view chosen => manual must go away
    $("#printing_month_manual").val("");
    closeManualSide();
    resetFormRowsOnly();
    clearMessages();
    ...
    $.post("printing-monthly-fetch.php", { month }, function (res) {
      ...
      $("#printing_missing_view_branches")
        .removeClass("d-none")
        .html("Something went wrong while loading the report. Try again.");
    });
  });

  /* =========================================================
     MANUAL MODE
     - when manual opens, view closes
  ========================================================== */
  $("#printing_month_manual").on("change", function () {
    const month = ($(this).val() || "").trim();

    // manual chosen => view must go away
    $("#printing_month_view").val("");
    closeViewSide();
    clearMessages();
    resetFormRowsOnly();
    ...
    $.post("printing-monthly-fetch.php", { month }, function (res) {
      ...
      $("#printing_missing_manual_branches")
        .removeClass("d-none")
        .html("Couldn’t load missing/provision info. Try again.");
    });
  });

  /* =========================================================
     Branch blur: existing / new handling
  ========================================================== */
  $(document).on("blur", ".printing_branch_code", function () {
    const row = $(this).closest("tr");
    const branch_code = ($(this).val() || "").trim();
    const month = ($("#printing_month_manual").val() || "").trim();
  ...
  /* =========================================================
     Save
  ========================================================== */
  $("#printing_save_entry").on("click", function () {
    const month = ($("#printing_month_manual").val() || "").trim();
    const row = $("#printing_entry_rows tr").last();

    const branch_code = (row.find(".printing_branch_code").val() || "").trim();
    const branch_name = (row.find(".printing_branch_name").val() || "").trim();
    const amount = (row.find(".printing_amount").val() || "").replace(/,/g, "").trim();
    ...
    $.post("printing-monthly-save.php",
      { month, branch_code, branch_name, amount, provision, provision_reason },
      function (res) {

        if (res.status === "success") {
          $("#printing_status_msg").html(`<div class='alert alert-success'>${res.message}</div>`);

          // refresh alert panel after save (so missing list updates)
      ...
      $("#printing_status_msg").html(`<div class='alert alert-danger'>Save failed (server/network).</div>`);
    });
  });
  ...
  /* =========================================================
     CSV Download
  ========================================================== */
  $("#printing_download_csv_btn").on("click", function () {
    ...
    const month = $("#printing_month_view").val()
      ? $("#printing_month_view").val().replace(/\s+/g, "_")
      : "Month";
