  ...
  <div class="container-fluid">
    <div class="card shadow bg-white rounded p-4">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <h5 class="text-primary">Printing Budget vs Actual</h5>
...
<!-- Unique remarks modal for Printing -->
<div class="modal fade" id="printingRemarksModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
...
(function(){
  ...
  const FETCH_CANDIDATES = [
    BASE + 'printing-budget-fetch.php',
    BASE.replace(/[^/]+\/$/, '') + 'printing-budget-fetch.php'
  ];
  ...
  function loadPrintingReport(){
    ...
    const go = (urls) => fetchWithFallback(
      ...
      () => {
        ...
        console.error('Printing fetch failed on all paths:', urls);
      }
    );
  ...
  // Open remarks (scoped to printing modal)
  $('#contentArea').on('click' + NS, '.open-remarks', function(){
    const category = $(this).data('category');
    const record   = $(this).data('record');
    const $modal   = $('#printingRemarksModal');
  ...
  $('#contentArea').on('click' + NS, '#save-remark', function(){
    ...
    $.post(BASE + 'save-remark.php', { category, record, comment, recipients, origin_page: 'printing-overview.php' }, function(response){
  ...
  $('#contentArea').on('hidden.bs.modal' + NS, '#printingRemarksModal', function(){
