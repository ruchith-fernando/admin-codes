...
$(function(){
  ...
  $("#vpn_month_view").on("change", function(){
    ...
    $.ajax({
      ...
      error: function(xhr){
        $("#vpn_status_msg").html(`<div class="alert alert-danger">❌ AJAX ${xhr.status}: ${xhr.statusText}</div>`);
  ...
  $("#vpn_save_entry").on("click", function(){
    ...
    if (!month || !amount) {
      $("#vpn_status_msg").html('<div class="alert alert-danger">Month and amount are required.</div>');
      return;
    }
    ...
    $.ajax({
      ...
      success: function(res){
        if (res.success) {
          ...
          $("#vpn_status_msg").html(`<div class="alert alert-danger">${res.message || 'Save failed'}</div>`);
        }
      },
      error: function(xhr){
        $("#vpn_status_msg").html(`<div class="alert alert-danger">❌ AJAX ${xhr.status}: ${xhr.statusText}</div>`);
  ...
  $("#vpn_download_csv_btn").on("click", function(){
    ...
    const month = $("#vpn_month_view").val()?$("#vpn_month_view").val().replace(/\s+/g,'_'):'Month';
