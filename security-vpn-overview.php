  ...
  <div class="container-fluid">
    <div class="card shadow bg-white rounded p-4">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <h5 class="text-primary">Security VPN Budget vs Actual</h5>
...
<!-- Remarks modal -->
<div class="modal fade" id="securityVPNRemarksModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
...
<script>
(function(){
  const NS = '.page.securityVPN';
  const BASE = '<?= htmlspecialchars(rtrim(dirname($_SERVER["PHP_SELF"]), "/")."/", ENT_QUOTES) ?>';
  const FETCH_CANDIDATES = [
    BASE + 'security-vpn-budget-fetch.php',
    BASE.replace(/[^/]+\/$/, '') + 'security-vpn-budget-fetch.php'
  ];
  ...
  $.ajaxSetup({
    cache: false,
    beforeSend: jqXHR => window.__xhrPool.push(jqXHR),
    complete: jqXHR => window.__xhrPool = window.__xhrPool.filter(x => x !== jqXHR)
  });
  ...
  function loadSecurityVPNReport(){
  ...
  $('#contentArea').on('click'+NS,'#back-to-dashboard',function(){
    ...
    $.get(DASHBOARD_URL,res=>$('#contentArea').html(res));
  });
  ...
  $('#contentArea').on('click'+NS,'#update-selection',function(){
    ...
    $.post(BASE+'update-dashboard-selection.php',{data:selected},function(res){
      if(res.status==='success'){
        ...
        loadSecurityVPNReport();
      }else{
        flash($('#page-alert'),'danger','❌ Failed to update: '+(res.message||'Unknown error'));
      }
    },'json').fail(()=>flash($('#page-alert'),'danger','❌ Server error occurred while updating.'));
  });

  $('#contentArea').on('click'+NS,'.open-remarks',function(){
    const category=$(this).data('category');
    const record=$(this).data('record');
    const $modal=$('#securityVPNRemarksModal');
    ...
    $.post(BASE+'get-remarks.php',{category,record},function(data){
      ...
      if(Array.isArray(data)&&data.length){
        data.forEach(item=>{
          html+=`<div><strong>${item.hris_id}</strong> <small class="text-muted">${item.commented_at}</small> <span class="badge bg-secondary ms-2">SR: ${item.sr_number}</span><br>${item.comment}</div><hr>`;
        });
      }else{
    ...
    },'json').fail(()=>flash($('#page-alert'),'danger','❌ Failed to load remarks.'));
  });
  ...
  $('#contentArea').on('click'+NS,'#save-remark',function(){
    ...
    $.post(BASE+'save-remark.php',{category,record,comment,recipients,origin_page:'security-vpn-overview.php'},function(response){
    ...
    },'json').fail(()=>flash($alertBox,'danger','❌ Server error occurred while saving.'));
  });

  $('#contentArea').on('change'+NS,'.month-checkbox',recalcSelectionTotals);

  $('#contentArea').on('hidden.bs.modal'+NS,'#securityVPNRemarksModal',function(){
