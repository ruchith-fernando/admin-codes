<?php
// slt-branch-report.php  (NEW)
// Page: Month selector + table area. Uses ajax-slt-branch-report.php and slt-branch-report-export-csv.php
ob_start();
...
// Build month list (distinct by bill_period_start’s month)
$months = [];
$qm = "SELECT DATE_FORMAT(bill_period_start,'%Y-%m-01') AS month_start
       FROM tbl_admin_slt_monthly_data
...
<div class="content font-size">
  <div class="container-fluid">
    <div class="card shadow bg-white rounded p-4">
      <h5 class="mb-4 text-primary">Mobile Bill Report - By Branch</h5>
...
(function (w, $) {
  ...
  function loadTableForMonth(monthYMs) {
    ...
    $.ajax({
      url: 'ajax-slt-branch-report.php', // NEW endpoint
      method: 'GET',
      data: { month: monthYMs }
    })
    .done(function (html) {
  ...
  // Export button uses current month
  w.exportData = function exportData(type){
    const msel = document.getElementById('monthSelect');
    const m = msel && msel.value ? msel.value : '';
    const url = (type === 'excel') ? 'slt-branch-report-export-csv.php' : 'slt-branch-report-export-csv.php';
