  ...
  <div class="container-fluid">
    <div class="card shadow bg-white rounded p-4">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <h5 class="text-primary">Courier Budget vs Actual</h5>
...
<!-- Unique remarks modal for courier -->
<div class="modal fade" id="courierRemarksModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
...
(function(){
  ...
  const FETCH_CANDIDATES = [
    BASE + 'tea-branch-budget-fetch.php',
    BASE.replace(/[^/]+\/$/, '') + 'tea-branch-budget-fetch.php'
  ];
  ...
  function loadcourierReport(){
    ...
    const go = (urls) => fetchWithFallback(
      ...
      () => {
        ...
        console.error('courier fetch failed on all paths:', urls);
      }
    );
  ...
  // Open remarks (scoped to courier modal)
  $('#contentArea').on('click' + NS, '.open-remarks', function(){
    const category = $(this).data('category');
    const record   = $(this).data('record');
    const $modal   = $('#courierRemarksModal');
  ...
  $('#contentArea').on('click' + NS, '#save-remark', function(){
    ...
    $.post(BASE + 'save-remark.php', { category, record, comment, recipients, origin_page: 'tea-branch-overview.php' }, function(response){
  ...
  $('#contentArea').on('hidden.bs.modal' + NS, '#courierRemarksModal', function(){
