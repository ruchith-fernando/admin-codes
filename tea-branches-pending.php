...
if (isset($_POST['log_month_click']) && isset($_POST['month'])) {
    ...
    userlog("📅 Tea Pending Month selected | Month: $month | HRIS: $hris | User: $user | IP: $ip");
...
");
$months=[];
while($m=mysqli_fetch_assoc($month_q)) $months[]=$m['month_applicable'];
?>

<div class="content font-size">
  <div class="container-fluid mt-4">
    <div class="card shadow bg-white rounded p-4">
      <h5 class="text-primary mb-3">Tea Branches — Pending Approvals</h5>
      ...
      <div class="mb-3">
        ...
        <select id="tea_pending_month_select" class="form-select">
      ...
      <div id="teaAlertContainer"></div>
      <div id="tea_pending_table_container" class="table-responsive mt-3"></div>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
    ...
    <form id="teaRejectForm">
      <div class="modal-body">
        ...
        <p>Rejecting record for <strong id="tea_reject_branch"></strong> — <span id="tea_reject_month"></span></p>

        <div class="mb-3">
          <label class="form-label">Reason</label>
          <select name="rejection_reason" id="tea_rejection_reason" class="form-select" required>
...
<div class="modal fade" id="teaApproveConfirmModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
    ...
    <div class="modal-footer">
      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
      <button type="button" id="teaConfirmApproveBtn" class="btn btn-success">Confirm Approve</button>
    </div>
  </div></div>
</div>
...
(function(){
  ...
  const approveModal = new bootstrap.Modal('#teaApproveConfirmModal');

  document.getElementById("tea_pending_month_select").addEventListener("change", function(){
    const month = this.value;
    document.getElementById("tea_pending_table_container").innerHTML = "";
    document.getElementById("teaApproveAllBtn").classList.add("d-none");

    if(month){
      fetch("tea-branches-pending.php", {
        method:"POST",
    ...
    fetch("tea-branches-pending-load.php", {
      method:"POST",
      headers:{ "Content-Type":"application/x-www-form-urlencoded" },
    ...
    .then(r=>r.text())
    .then(html=>{
      document.getElementById("tea_pending_table_container").innerHTML = html;
  ...
  document.body.addEventListener("click", e=>{
    ...
    fetch("tea-branches-approve-single.php", {
      method:"POST",
      headers:{ "Content-Type":"application/x-www-form-urlencoded" },
  ...
  document.body.addEventListener("click", e=>{
    ...
    document.getElementById("tea_reject_branch").textContent = btn.dataset.branch;
    document.getElementById("tea_reject_month").textContent = btn.dataset.month;
    rejectModal.show();
  });

  document.getElementById("tea_rejection_reason").addEventListener("change", function(){
  ...
  document.getElementById("teaRejectForm").addEventListener("submit", e=>{
    ...
    fetch("tea-branches-reject.php", { method:"POST", body:fd })
      .then(r=>r.json())
      .then(data=>{
        rejectModal.hide();
        showAlert(data.status, data.message);
  ...
  document.getElementById("teaApproveAllBtn").addEventListener("click", ()=>approveModal.show());

  document.getElementById("teaConfirmApproveBtn").addEventListener("click", ()=>{
    approveModal.hide();

    const ids = Array.from(document.querySelectorAll(".tea-approve-btn")).map(b=>b.dataset.id);
    if(ids.length === 0) return showAlert("info","No records to approve.");

    fetch("tea-branches-bulk-approve.php", {
      method:"POST",
      headers:{ "Content-Type":"application/x-www-form-urlencoded" },
  ...
  function updateApproveAllButton(){
    const remaining = document.querySelectorAll(".tea-approve-btn").length;
    if(remaining === 0) document.getElementById("teaApproveAllBtn").classList.add("d-none");
  }
  ...
  function showAlert(type,msg){
    ...
    document.getElementById('teaAlertContainer').prepend(alert);
  ...
  function showPDFDownloadForm(url){
    ...
    `;
    document.getElementById("teaAlertContainer").prepend(box);
    setTimeout(() => {
