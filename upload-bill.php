...
<div class="content font-size">
  <div class="container-fluid">
    <div class="card">
      <h5>Upload Mobile Bill PDF</h5>

      <!-- Result area (same pattern as SLT page) -->
      <div id="uploadResult" class="result-block" style="display:none"></div>

      <!-- novalidate stops browser tooltip; we show our own alert instead -->
      <form id="billUploadForm" enctype="multipart/form-data" action="process-bill.php" method="post" novalidate>

        <!-- ===== Pre-upload Checklist (added) ===== -->
        <div class="checklist-block">
          <div class="checklist-title">Pre-upload Checklist - Stick to this order</div>

          <div class="check-item">
            <div class="check-label">1. Uploaded the active/inactive employee list</div>
          ...
          <div class="check-item">
            <div class="check-label">2. Uploaded the latest connections</div>
            <div class="toggle-group">
          ...
          <div id="checklistMsg" class="tiny-hint" style="color:#b71c1c">Please mark both as “Yes” to enable the upload field.</div>
        </div>
        <!-- ===== /Pre-upload Checklist ===== -->

        <div class="mb-3">
          <label class="form-label" for="pdf_file">Choose PDF File</label>
          <input class="form-control" type="file" id="pdf_file" name="pdf_file" accept="application/pdf,.pdf" required disabled />
          <div class="mt-2" style="font-size:.9rem;color:#555">Please upload a single PDF file. Month is auto-detected from the bill data.</div>
        </div>
...
(function(){
  ...
  function updateChecklistState(){
    if (bothYes()){
      ...
      $msg.css('color','#1b5e20').text('✅ All set. You can upload the PDF now.');
    } else {
  ...
  // Clear previous error when a file is chosen
  $file.on('change', function(){ $file.removeClass('is-invalid'); $result.hide().empty(); });

  $form.on('submit', function(e){
    e.preventDefault();

    // Block submission unless both toggles are YES (added)
    if (!bothYes()){
    ...
    if(!isPdf){ showError('Only PDF files are allowed.'); return; }
    ...
    $.ajax({
      ...
      success: function(html){
        ...
        setTimeout(function(){
          // Show whatever the server returned (may already include alerts)
          showResult(html || "<div class='alert alert-success'><b>✅ Uploaded.</b></div>");
        }, 120);
        $form.trigger('reset');
        updateChecklistState(); // keep file disabled again after reset
      },
      error: function(x){
