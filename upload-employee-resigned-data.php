...
<!-- Global loader (same look & feel as your other upload page) -->
<div id="globalLoader">
...
<style>
  ...
  /* Clean card + form styles (matches your CDMA upload layout) */
...
<div class="content font-size">
  <div class="container-fluid">
    <div class="card shadow bg-white rounded p-4">
      <h5 class="mb-4 text-primary">Upload Inactive Employee CSV</h5>

      <!-- Inline results (replaces modal) -->
      <div id="uploadResult" class="result-block"></div>

      <form id="resignedCsvForm" enctype="multipart/form-data" novalidate>
        <div class="mb-3">
          <label for="csvFile" class="form-label">Select CSV File</label>
          <input type="file" name="csvFile" id="csvFile" class="form-control" accept=".csv" required>
          <div class="mt-2 hint">
            Upload the latest list of <b>resigned / inactive</b> employees. 
          </div>
        </div>

        <button type="submit" class="btn btn-success" id="resignedUploadBtn">
...
<script>
  (function () {
    const $form   = $('#resignedCsvForm');
    const $file   = $('#csvFile');
    const $result = $('#uploadResult');
    const $loader = $('#globalLoader');
    const $btn    = $('#resignedUploadBtn');

    $form.on('submit', function (e) {
      e.preventDefault();

      if (!$file.prop('files')[0]) {
        $result.html("<div class='alert alert-danger fw-bold'>❌ Please choose a CSV file.</div>").slideDown();
        return;
      }
      ...
      $.ajax({
        url: 'ajax-upload-employee-resigned-data.php', // ✅ unchanged endpoint
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false
  ...
  // Kept from your original snippet (if you use it elsewhere)
  function toggleMenu() {
