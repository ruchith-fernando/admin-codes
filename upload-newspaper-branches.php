...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv'])) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        // strip hidden BOM chars if present
        $branch_code = preg_replace('/\x{FEFF}/u', '', $branch_code);

        if ($branch_code === "" || $branch_name === "") {
            error_log("Skipped line $line: empty code or name");
            continue;
        }
        ...
        // log the row to error_log for debugging
        error_log("Processing row $line: code=[$branch_code], name=[$branch_name]");

        $check = mysqli_query($conn, "SELECT id FROM tbl_admin_branch_newspaper WHERE branch_code = '$branch_code' LIMIT 1");
        if ($check === false) {
            echo json_encode(["status" => "error", "message" => "Check query failed: " . mysqli_error($conn)]);
            exit;
        }

        if (mysqli_num_rows($check) > 0) {
            $sql = "UPDATE tbl_admin_branch_newspaper 
                       SET branch_name = '$branch_name', updated_at = NOW() 
            ...
            $sql = "INSERT INTO tbl_admin_branch_newspaper (branch_code, branch_name) 
...
<head>
<meta charset="utf-8">
<title>Upload Water Branches CSV</title>
<style>
...
<div class="content font-size">
  <div class="container-fluid">
    <div class="card">
      <h5>Upload News Paper Branches CSV</h5>
      <div id="uploadResult" class="result-block" style="display:none"></div>
      <form id="csvUploadForm" enctype="multipart/form-data" action="upload-water-branches.php" method="post" novalidate>
        <div class="mb-3">
          ...
          <div class="mt-2" style="font-size:.9rem;color:#555">Expected columns: Branch Code, Branch Name</div>
        </div>
...
<script>
(function(){
  const $form=$('#csvUploadForm'),$result=$('#uploadResult');
  ...
  $form.on('submit',function(e){
    ...
    $btn.prop('disabled',true); $wrap.show(); $label.text('Uploading…');
    ...
    $.ajax({
      ...
      complete:function(){ $btn.prop('disabled',false); setTimeout(resetProgress,600); }
    });
  });
})();
</script>
