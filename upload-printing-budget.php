...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv'])) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        $check = mysqli_query($conn, "SELECT id FROM tbl_admin_budget_printing 
                                      WHERE branch_code = '$branch_code' AND budget_year = '$budget_year' LIMIT 1");

        if ($check && mysqli_num_rows($check) > 0) {
            $sql = "UPDATE tbl_admin_budget_printing 
                       SET branch_name = '$branch_name', amount = '$amount', uploaded_at = NOW() 
                     WHERE branch_code = '$branch_code' AND budget_year = '$budget_year'";
            if (mysqli_query($conn, $sql)) {
                $updated++;
            }
        } else {
            $sql = "INSERT INTO tbl_admin_budget_printing (branch_code, branch_name, budget_year, amount) 
                    VALUES ('$branch_code', '$branch_name', '$budget_year', '$amount')";
            if (mysqli_query($conn, $sql)) {
                $inserted++;
            }
        }
...
<head>
<meta charset="utf-8">
<title>Upload Printing Budget CSV</title>
<style>
...
<div class="content font-size">
  <div class="container-fluid">
    <div class="card">
      <h5>Upload Printing Budget CSV</h5>
      <div id="uploadResult" class="result-block" style="display:none"></div>
      <form id="csvUploadForm" enctype="multipart/form-data" action="upload-printing-budget.php" method="post" novalidate>
        <div class="mb-3">
          ...
          <div class="mt-2" style="font-size:.9rem;color:#555">Expected columns: Branch Code, Branch Name, Budget Year, Amount</div>
        </div>
