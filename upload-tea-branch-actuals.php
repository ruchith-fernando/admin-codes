...
$table = "tbl_admin_actual_branch_gl_tea";
$log_table = "tbl_admin_actual_branch_gl_tea_upload_log";
$GL_DESC_OVERRIDE = "TEA";
...
/**
 * Compute normalized hash so "same information" is detected even if filename/order changes.
 * IMPORTANT: GL DESCRIPTION is forced by $glDescOverride (CSV value ignored).
 */
function compute_normalized_hash($filePath, $glDescOverride) {
    ...
    while (($data = fgetcsv($handle, 5000, ",")) !== false) {
        ...
        $gl_description       = $glDescOverride; // force
        ...
        if ($gl_code === "" || $enterd_brn === "" || $dateoftran_raw === "") continue;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv'])) {
    if (ob_get_length()) { ob_clean(); } // CLEAN OUTPUT so JSON is pure
    ...
    list($normalized_hash, $hash_err) = compute_normalized_hash($file, $GL_DESC_OVERRIDE);
    if ($hash_err) {
    ...
    while (($data = fgetcsv($handle, 5000, ",")) !== false) {
        ...
        $gl_code              = trim(get_field($data, $hmap, "GL CODE", 0));
        $gl_description       = $GL_DESC_OVERRIDE;
        ...
        if ($gl_code === "" || $enterd_brn === "" || $dateoftran_raw === "") {
            $invalid++;
            continue;
        }
...
<head>
<meta charset="utf-8">
<title>Upload Tea Branch GL Actuals CSV</title>
<style>
...
<div class="content font-size">
  <div class="container-fluid">
    <div class="card">
      <h5 class="text-primary mb-3">Upload Tea Branch GL Actuals CSV</h5>
      ...
      <form id="csvUploadForm"
        ...
        <div class="mb-3">
          ...
          <div class="form-text">
            ...
            <b>GL DESCRIPTION is forced to TEA</b>.
...
(function(){
  ...
  $form.on('submit',function(e){
    ...
    $.ajax({
      ...
      // IMPORTANT: treat as text and parse ourselves (prevents parsererror)
      dataType:'text',
      ...
      success:function(resp){
        let json;
        try{
          json = JSON.parse(String(resp).replace(/^\uFEFF/, '').trim());
        } catch(e){
          showError(resp || 'Upload failed.');
          return;
        }
