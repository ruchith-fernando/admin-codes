...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv'])) {
    ...
    $entered_hris = mysqli_real_escape_string($conn, $_SESSION['hris']);
    $entered_name = mysqli_real_escape_string($conn, $_SESSION['name']);
    $entered_by   = mysqli_real_escape_string($conn, $_SESSION['user']['user_level'] ?? 'unknown');
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        $amount          = trim($data[2] ?? "");
        $month_applicable= trim($data[3] ?? "");

        if ($branch_code==="" || $branch_name==="" || $amount==="" || $month_applicable==="") continue;

        $branch_code      = mysqli_real_escape_string($conn, $branch_code);
        $branch_name      = mysqli_real_escape_string($conn, $branch_name);
        $amount           = mysqli_real_escape_string($conn, $amount);
        $month_applicable = mysqli_real_escape_string($conn, $month_applicable);

        $check = mysqli_query($conn, "
            SELECT id FROM tbl_admin_actual_water 
            WHERE branch_code = '$branch_code'
              AND month_applicable = '$month_applicable'
            LIMIT 1
        ");
        ...
        if ($check && mysqli_num_rows($check) > 0) {
            ...
            $sql = "
                INSERT INTO tbl_admin_actual_water 
                    (branch_code, branch, total_amount, month_applicable,
                     entered_hris, entered_name, entered_by, entered_at, is_provision)
                VALUES
                    ('$branch_code', '$branch_name', '$amount', '$month_applicable',
                     '$entered_hris', '$entered_name', '$entered_by', NOW(), 'no')
            ";
            if (mysqli_query($conn, $sql)) $inserted++;
        }

        userlog("📦 Water Actual Upload | HRIS: $entered_hris | Name: $entered_name | Branch: $branch_name | Code: $branch_code | Month: $month_applicable | Uploaded: $now");
    }
...
<html>
<head>
<title>Upload Water Actuals CSV</title>
...
<div class="content font-size">
  <div class="container-fluid">
    <div class="card">
      ...
      <h5>Upload Water Actuals CSV</h5>

      <div id="uploadResult" class="result-block" style="display:none"></div>

      <form id="csvUploadForm" enctype="multipart/form-data" method="post" novalidate>
        <div class="mb-3">
          <label class="form-label">Choose CSV File</label>
          <input type="file" class="form-control" id="csv_file" name="csv" accept=".csv,text/csv" required>
          <div style="font-size:.9rem;color:#555;margin-top:6px">
            Expected columns: <b>Branch Code, Branch, Amount, Applicable Month Example - April 2025</b>
          </div>
        </div>
...
(function(){
  ...
  function showError(msg){ $file.addClass('is-invalid'); showResult("<div class='alert alert-danger'><b>❌ "+msg+"</b></div>"); }
    ...
    $.ajax({
      ...
      success:function(resp){
        if(resp.status==='success'){
          showResult("<div class='alert alert-success'><b>✅ Upload complete.</b> Inserted: "+resp.inserted+" | Updated: "+resp.updated+"</div>");
        } else {
