...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv'])) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        // Check if branch already has a record for this year
        $check = mysqli_query($conn,
            "SELECT id FROM tbl_admin_budget_water 
             WHERE branch_code = '$branch_code' AND budget_year = '$budget_year' 
             LIMIT 1"
        );

        if ($check && mysqli_num_rows($check) > 0) {
            // Update existing
            $sql = "UPDATE tbl_admin_budget_water 
            ...
            // Insert new
            $sql = "INSERT INTO tbl_admin_budget_water 
...
<div class="content font-size">
  <div class="container-fluid">
    <div class="card">
      <h5>Upload Water Budget CSV</h5>
      <div id="uploadResult" class="result-block" style="display:none"></div>
      <form id="csvUploadForm" enctype="multipart/form-data" action="upload-water-budget.php" method="post" novalidate>
        <div class="mb-3">
          ...
          <div class="mt-2" style="font-size:.9rem;color:#555">Expected columns: <code>Branch Code, Branch Name, Budget Year - Example 2025, Amount - Without the 1000 separator</code></div>
        </div>
      ...
      success:function(resp){
        try{
          ...
          if(json.status==='success'){
            showResult("<div class='alert alert-success'><b>✅ Success.</b> Inserted: "+json.inserted+", Updated: "+json.updated+"</div>");
          } else {
            showResult("<div class='alert alert-danger'><b>❌ "+json.message+"</b></div>");
          }
        }catch(e){ showResult(resp); }
      },
