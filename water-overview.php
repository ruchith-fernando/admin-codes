...
<!-- Unique remarks modal for Water -->
<div class="modal fade" id="waterRemarksModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
...
(function(){
  ...
  const FETCH_CANDIDATES = [
    BASE + 'water-budget-fetch.php',
    BASE.replace(/[^/]+\/$/, '') + 'water-budget-fetch.php'
  ];
  ...
  function loadWaterReport(){
    ...
    const go = (urls) => fetchWithFallback(
      ...
      () => {
        ...
        console.error('Water fetch failed on all paths:', urls);
      }
    );
  ...
  // Open remarks (scoped to water modal)
  $('#contentArea').on('click' + NS, '.open-remarks', function(){
    const category = $(this).data('category');
    const record   = $(this).data('record');
    const $modal   = $('#waterRemarksModal');
  ...
  $('#contentArea').on('click' + NS, '#save-remark', function(){
    ...
    $.post(BASE + 'save-remark.php', { category, record, comment, recipients, origin_page: 'water-overview.php' }, function(response){
  ...
  $('#contentArea').on('hidden.bs.modal' + NS, '#waterRemarksModal', function(){
